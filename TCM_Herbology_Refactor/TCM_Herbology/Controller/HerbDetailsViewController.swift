...
class HerbDetailsViewController: UITableViewController {
    ...
    func cellActionHandler(_ list:[String]?,_ idx:Int) {
        if let list{
            let data = list[idx]
    ...
    func generalInfoCell(indexPath: IndexPath)-> UITableViewCell{
    ...
    func fufanInfoCell(indexPath: IndexPath)-> UITableViewCell{
        let data = displaySettings[indexPath.row]
        let title = "\(data.title)"
        let subtitle = "\(getHerbData(key: data.key))"
        var cell = tableView.dequeueReusableCell(withIdentifier: "InnerTableViewCell", for: indexPath)
        if let cell1 = cell as? InnerTableViewCell{
            ...
            cell1.actionDelegate = cellActionHandler
        }
        return cell
    }
    func expandableInfoCell(indexPath: IndexPath)-> UITableViewCell{
        let data = displaySettings[indexPath.row]
        let title = "\(data.title)"
        let subtitle = "\(getHerbData(key: data.key))"
        var cell = tableView.dequeueReusableCell(withIdentifier: "MyExpandableTableViewCell", for: indexPath)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if data.type == .general_info {
            cell = generalInfoCell(indexPath: indexPath)
        }
        else if data.type == .fufan{
            cell = fufanInfoCell(indexPath: indexPath)
        }
        else if data.type == .list_info{
            cell = expandableInfoCell(indexPath: indexPath)
        }else{
